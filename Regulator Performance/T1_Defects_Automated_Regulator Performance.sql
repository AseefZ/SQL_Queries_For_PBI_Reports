...
SELECT
	 DEF.[ASSET] AS ASSET_ID
        ,CASE WHEN (Len(DEF.Asset) - Len(Replace(DEF.Asset, '-', ''))) > 2 THEN 
        LEFT(DEF.Asset, CHARINDEX('-', DEF.Asset) - 1) + '-' + SUBSTRING(DEF.Asset,LEN(LEFT(DEF.Asset, 
        CHARINDEX('-', DEF.Asset)+1)),LEN(DEF.Asset) - LEN(LEFT(DEF.Asset, CHARINDEX('-', DEF.Asset))) - LEN(RIGHT(DEF.Asset, 
        CHARINDEX('-', (REVERSE(DEF.Asset)))))) ELSE DEF.Asset END AS "Asset_ID_UNIQUE"        
        ,DEF.[Defect] AS DEFECT_CODE
